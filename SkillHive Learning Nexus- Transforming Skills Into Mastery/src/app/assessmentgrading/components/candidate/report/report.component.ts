...
export class ReportComponent implements OnInit{
  ...
  ngOnInit():void{
    ...
    const diffMs = Math.abs(history.state.assessmentData.startAt.getTime() - history.state.assessmentData.completionAt.getTime())
    ...
    console.log(history.state.assessmentData)
  ...
  calculateMarks(questions : any){
    const marks = ( questions.filter((data:any)=> data.userAnswer === data.answer ) / questions.length )*100
